...
import 'package:tour_guide_app/features/train_booking/presentation/pages/train_ticket.page.dart';
import 'package:tour_guide_app/features/car_rental/presentation/widgets/payment_method_selector.widget.dart';

class TrainBookingConfirmationPage extends StatefulWidget {
  ...
  const TrainBookingConfirmationPage({
    Key? key,
  ...
  @override
  State<TrainBookingConfirmationPage> createState() => _TrainBookingConfirmationPageState();
}

class _TrainBookingConfirmationPageState extends State<TrainBookingConfirmationPage> {
    ...
    Navigator.of(context, rootNavigator: true).push(
      MaterialPageRoute(
        builder: (context) => TrainTicketPage(
          ...
          bookingCode: 'TRAIN${DateTime.now().millisecondsSinceEpoch}',
        ),
      ),
    );
  }
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(Icons.event_seat_rounded, size: 24.r, color: AppColors.primaryWhite),
              ),
              SizedBox(width: 12.w),
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4.h),
                    Text(
                      widget.selectedSeats.map((s) => "Toa ${s['coach']}-${s['seat'] + 1}").join(', '),
                      style: Theme.of(context).textTheme.titleSmall,
                    ),
                  ],
                ),
  ...
  Widget _buildPriceBreakdown(double totalPrice) {
    ...
    final insurance = 20000.0 * widget.selectedSeats.length;
    final total = ticketPrice + serviceFee + insurance;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 12.h),
          _buildPriceRow('Giá vé (${widget.selectedSeats.length} chỗ)', ticketPrice),
          SizedBox(height: 8.h),
  ...
  Widget _buildBottomBar(double totalPrice) {
    final serviceFee = totalPrice * 0.05;
    final insurance = 20000.0 * widget.selectedSeats.length;
    final total = totalPrice + serviceFee + insurance;
